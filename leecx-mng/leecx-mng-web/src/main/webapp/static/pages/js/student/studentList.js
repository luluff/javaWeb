...
var reloadItemGrid = function() {
	...
	jqGrid.jqGrid('setGridParam',{datatype:'json'}).setGridParam({ 
		page: 1,
        url: hdnContextPath + "/student/getStudentList.action",
    }).trigger("reloadGrid");
}
...
var deleteItem = function(itemId) {
	...
	swal({
		...
		if (isConfirm) {
			...
			$.ajax({
		    	url: $("#hdnContextPath").val() + "/student/deleteItem.action",
		    	type: "POST",
		    	async: false,
		    	data: {"itemId": itemId},
...
var DemoItemList = function () {
	...
	var handleDemoItemList = function() {
        ...
        jqGrid.jqGrid({  
            ...
            styleUI: 'Bootstrap',
            datatype: "json",  
            colNames: ['名字','电话','头像','操作'],  
            colModel: [  
                { name: 'name', index: 'name', width: 20, sortable: false },  
                { name: 'telephone', index: 'telephone', width: 30, sortable: false },
                { name: 'icon', index: 'icon', width: 30, sortable: false,
                	formatter: function (cellvalue, options, rowObject) {
                		
                		var url=iconServer+cellvalue;
                		var string="<img src='"+url+"' height='100' width='100' />";
                		return string;
                	}
                },
                { width: 30, sortable: false,
                	formatter:function(cellvalue, options, rowObject) {
    			    	...
    			    	var btnDetail = '<a class="btn btn-outline blue-chambray" id="ajaxDetailItemInfoLink" name="ajaxDetailUserInfoLink" data-url="' + hdnContextPath + '/student/showItemInfoPage.action?itemId=' + itemId + '" data-toggle="modal" style="padding: 1px 3px 1px 3px;">详情</a>';
    			    	
    			    	var btnModify = '<a class="btn btn-outline blue-chambray" id="ajaxModifyItemLink" name="ajaxModifyItemLink" data-url="' + hdnContextPath + '/student/showModifyItemPage.action?itemId=' + itemId + '" data-toggle="modal" style="padding: 1px 3px 1px 3px;">编辑</a>';
